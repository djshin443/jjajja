...
function showTitleScreen() {
    ...
    if (gameContainer) {
        ...
        // PWA 모드에서는 gameContainer도 상태바 영역까지 확장
        if (isStandalone && /iPhone|iPad|iPod/.test(navigator.userAgent)) {
            gameContainer.style.position = 'fixed';
            gameContainer.style.top = '0';
            gameContainer.style.paddingTop = '0';
            gameContainer.style.background = 'transparent'; // 타이틀 화면 동안 배경 숨김
        }
    }
    ...
    // 인라인 스타일 - PWA 모드는 CSS 미디어 쿼리에서 처리
        ...
        style.textContent = `
            ...
            @supports (-webkit-touch-callout: none) {
                ...
                @media all and (display-mode: standalone) {
                    /* gameContainer도 상태바 영역까지 확장 (파란 여백 방지) */
                    #gameContainer {
                    ...
                    #titleScreen {
                        /* 상태바 영역까지 완전히 채우기 */
                        /* env()가 0이면 폴백값 사용 (iPhone X 이상: 44px, 이하: 20px) */
                        position: fixed !important;
                        top: 0 !important;
                        top: -env(safe-area-inset-top, 20px) !important;
                        left: 0 !important;
                        right: 0 !important;
                        bottom: auto !important;  /* bottom 제거하여 height와 충돌 방지 */
                        width: 100vw !important;
                        height: 100vh !important;
                        height: calc(100vh + env(safe-area-inset-top, 20px)) !important;
                        min-height: calc(100vh + env(safe-area-inset-top, 20px)) !important;
